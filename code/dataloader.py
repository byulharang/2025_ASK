...
class PairedDataset(Dataset):
    def __init__(self, dataset, depth_dt, transform=None, n_fft=256, hop=16):
        self.file_list = []
        self.root_dir = dataset
        self.depth_dir = depth_dt
        ...
        for room_name in self.room_dirs:
            ...
            depth_room_path = os.path.join(self.depth_dir, room_name)
            ...
            depth_file_per_room = os.listdir(depth_room_path)
            depth_file_sorted = sorted(
                depth_file_per_room, key=lambda x: int(x.split("_")[0])
            )
            for file in depth_file_sorted:
                depth_files.append(os.path.join(depth_room_path, file))
            ...
            if (
                ...
                for idx, (img_file, depth_file, audio_file) in enumerate(
                    zip(image_files, depth_files, audio_files)
                ):
                    mic_info = mic_info_list[idx]
                    self.file_list.append((img_file, depth_file, audio_file, mic_info))
            else:
                raise ValueError(
    ...
    def __getitem__(self, idx):
        img_path, depth_path, audio_path, mic_info = self.file_list[idx]
        ...
        depth = torch.tensor(depth, dtype=torch.float32)  # Tensor
        ...
        return masked_image, image, rirs_1D_tensor, depth, mic_info

...
class Trainer:
    def __init__(
        self, model, train_loader, val_loader, test_loader, optimizer, device, max_norm=1.0, accumulation_steps=16
    ):
        self.model = model
    ...
    def train(self, epoch, total_epochs, exp_order, metric=False):
        ...
        for batch_idx, (masked_img, img, rir, depth, mic_info) in enumerate(
            ...
            loss = self.model(masked_img, img, rir, depth, mic_info, exp_order, metric)
    ...
    def validation(self, epoch, total_epochs, exp_order, metric=False):
        ...
        with torch.no_grad():
            for batch_idx, (masked_img, img, rir, depth, mic_info) in enumerate(
                ...
                loss = self.model(
                    masked_img, img, rir, depth, mic_info, exp_order, metric
                )
                loss = loss.mean()
